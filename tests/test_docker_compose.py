...
class TestDockerfileCompose:
    ...
    def test_docker_compose_file(self):
        try:
            with open(f'{os.path.join(infra_dir_path, "../infra/docker-compose.yml")}', 'r') as f:
                docker_compose = f.read()
        except FileNotFoundError:
            assert False, f'Проверьте, что в директорию {infra_dir_path} добавлен файл `docker-compose.yml`'

        assert re.search(r'image:\s+postgres:', docker_compose), (
            'Проверьте, что  в файл docker-compose.yml добавлен образ postgres:latest'
        )
